...
def calc_single_image_hotspot_tif(file_path: str, threshold: float, output_path: str, uint8: bool = False) -> str:
    ...
    """
    # Read the original image's profile to preserve georeference
    import os
    import numpy as np
    import rasterio

    with rasterio.open(file_path) as _src:
    ...
    # Save as GeoTIFF (keep nodata only on the first band per rasterio limitation)
    if mask.ndim == 3:
        mask = mask[:, :, 0]
    profile.update(
    ...
    profile["nodata"] = None if nodata is None else nodata
...
def calculate_tif_average(file_list: list[str], output_path: str, uint8: bool = False) -> str:
    ...
    # Read first file to get basic info + profile (preserve georeference)
    with rasterio.open(file_list[0]) as src:
        profile = src.profile.copy()
        first_img = src.read()  # (bands, H, W)

    # Initialize accumulator
    sum_img = first_img.astype(np.float64)
    count = len(file_list)

    # Read and accumulate remaining images (all bands)
    ...
    if uint8:
        for band in range(avg_img.shape[0]):
            band_data = avg_img[band]
            min_val = np.min(band_data)
            max_val = np.max(band_data)
            if max_val > min_val:
                avg_img[band] = (band_data - min_val) / (max_val - min_val) * 255
            else:
                avg_img[band] = 0
        avg_img = avg_img.astype(np.uint8)

    # Save result
    data_type = "uint8" if uint8 else "float32"
    profile.update(dtype=data_type, count=avg_img.shape[0], compress="lzw")
    with rasterio.open(output_path, "w", **profile) as out_ds:
        out_ds.write(avg_img)
...
def calculate_tif_difference(image_a_path: str, image_b_path: str, output_path: str, uint8: bool = False) -> str:
    ...
    # Read both images (all bands)
    with rasterio.open(image_a_path) as src_a:
        img_a = src_a.read().astype(np.float64)
        profile = src_a.profile.copy()
    with rasterio.open(image_b_path) as src_b:
        img_b = src_b.read().astype(np.float64)

    # Check if images have same dimensions
    if img_a.shape != img_b.shape:
        raise ValueError(
            f"Images must have same dimensions. Image A: {img_a.shape}, Image B: {img_b.shape}"
        )
    ...
    # Convert to uint8 if needed
    if uint8:
        diff_min, diff_max = diff_img.min(), diff_img.max()
        if diff_max > diff_min:
            diff_img = (diff_img - diff_min) / (diff_max - diff_min) * 255
        else:
            diff_img = np.zeros_like(diff_img)
        diff_img = diff_img.astype(np.uint8)

    # Save result
    data_type = "uint8" if uint8 else "float64"
    profile.update(dtype=data_type, count=diff_img.shape[0], compress="lzw")
    with rasterio.open(output_path, "w", **profile) as out_ds:
        out_ds.write(diff_img)
    return f"Result save at {TEMP_DIR / output_path}"
...
def grayscale_to_colormap(image_path: str, save_name: str, cmap_name: str = 'viridis', preserve_geo: bool = False):
    ...
    if preserve_geo:
        # Save as RGB GeoTIFF
        save_path = os.path.splitext(str(save_path))[0] + '.tif'
        profile.update(dtype=rasterio.uint8, count=3, compress="lzw")
        with rasterio.open(save_path, "w", **profile) as out_ds:
            out_ds.write(np.transpose(color_img_uint8, (2, 0, 1)))
    else:
