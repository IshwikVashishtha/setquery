...
def ATI(
    ...
    def resample_to_reference(src_data: np.ndarray, src_profile: dict, ref_profile: dict) -> np.ndarray:
        ...
        Description:
            ...
            using bilinear interpolation (scipy.ndimage.zoom — replaces the GDAL bilinear warp,
            which has no Windows wheel for Python 3.13 in this repo's stack).
        ...
        scaled = zoom(
            src_data,
            (ref_profile['height'] / src_height, ref_profile['width'] / src_width),
            order=1,
        )
        return scaled.astype(src_data.dtype)
